...
class ChartWizardWidget(QtWidgets.QWidget):
    ...
    def new_chart(self) -> None:
        ...
        # 检查合约是否存在，如果不存在则记录日志但不阻止创建图表
        contract: ContractData | None = None
        if "LOCAL" not in vt_symbol:
            contract = self.main_engine.get_contract(vt_symbol)
            if not contract:
                self.main_engine.write_log(f"ChartWizard: 未找到合约 {vt_symbol}，将尝试从数据库加载数据")
        else:
            self.main_engine.write_log(f"ChartWizard: 创建本地图表 {vt_symbol}")

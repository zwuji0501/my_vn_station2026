...
class SqliteDatabase(BaseDatabase):
    ...
    def load_bar_data(
        ...
        for db_bar in s:
            # 处理datetime字段的类型转换问题
            if isinstance(db_bar.datetime, str):
                # 如果是字符串格式，尝试解析为datetime对象
                try:
                    # 假设字符串格式为ISO格式或时间戳字符串
                    dt = datetime.fromisoformat(db_bar.datetime.replace('Z', '+00:00'))
                except ValueError:
                    try:
                        # 尝试作为时间戳字符串处理
                        dt = datetime.fromtimestamp(float(db_bar.datetime), DB_TZ)
                    except (ValueError, TypeError):
                        # 如果都失败，使用当前时间作为fallback
    ...
    def load_tick_data(
        ...
        for db_tick in s:
            # 处理datetime字段的类型转换问题
            if isinstance(db_tick.datetime, str):
                # 如果是字符串格式，尝试解析为datetime对象
                try:
                    # 假设字符串格式为ISO格式或时间戳字符串
                    dt = datetime.fromisoformat(db_tick.datetime.replace('Z', '+00:00'))
                except ValueError:
                    try:
                        # 尝试作为时间戳字符串处理
                        dt = datetime.fromtimestamp(float(db_tick.datetime), DB_TZ)
                    except (ValueError, TypeError):
                        # 如果都失败，使用当前时间作为fallback
                        dt = datetime.now(DB_TZ)
            else:
                # 如果是正常的datetime对象
                dt = datetime.fromtimestamp(db_tick.datetime.timestamp(), DB_TZ)

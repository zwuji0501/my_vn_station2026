...
from ..constant import Direction, Exchange, Offset, OrderType, Status
...
class OrderStatsMonitor(BaseMonitor):
    """
    Monitor for order statistics data.
    """
    ...
    headers: dict = {
        "total_count": {"display": _("总数量"), "cell": BaseCell, "update": True},
        "traded_volume": {"display": _("成交数量"), "cell": BaseCell, "update": True},
        "cancelled_count": {"display": _("撤单数量"), "cell": BaseCell, "update": True},
    }

    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        """"""
        # Initialize attributes before calling super().__init__()
        # because BaseMonitor.__init__() calls init_ui()
        self.total_count: float = 0
        self.traded_volume: float = 0
        self.cancelled_count: int = 0
        self.all_orders: dict[str, OrderData] = {}

        super().__init__(main_engine, event_engine)

    def init_ui(self) -> None:
        """
        Initialize UI with single row display.
        """
        super().init_ui()
        ...
        # Initialize the stats row after table is set up
        self.update_stats_display()

    def process_event(self, event: Event) -> None:
        """
        Process order event and update statistics.
        """
        order: OrderData = event.data
        vt_orderid = order.vt_orderid

        # Store order data
        self.all_orders[vt_orderid] = order
    ...
    def calculate_stats(self) -> None:
        """
        Calculate order statistics from all stored orders.
        Total count = Traded volume + Cancelled order count
        """
        ...
        # Total count = traded volume + cancelled count
        self.total_count = self.traded_volume + self.cancelled_count

    def update_stats_display(self) -> None:
        """
        Update the statistics display in the table.
        """
        # Clear existing content
        self.clearContents()

        # Set the single row data
        self.setItem(0, 0, BaseCell(str(self.total_count), None))
        self.setItem(0, 1, BaseCell(str(self.traded_volume), None))
        self.setItem(0, 2, BaseCell(str(self.cancelled_count), None))

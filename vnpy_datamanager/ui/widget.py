...
class ManagerWidget(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        convert_tdx_button: QtWidgets.QPushButton = QtWidgets.QPushButton("转换TDX数据")
        convert_tdx_button.clicked.connect(self.convert_tdx_data)
    ...
    def convert_tdx_data(self) -> None:
        """转换TDX数据为CSV格式"""
        try:
            # 设置默认路径
            ...
            reply = QtWidgets.QMessageBox.question(
                ...
                f"源目录：{source_dir}\n"
                f"目标目录：{target_dir}\n\n"
                "这将把所有.lc1文件转换为CSV格式并转换为vnpy格式，是否继续？",
            ...
            # 显示进度对话框
            progress = QtWidgets.QProgressDialog("正在转换TDX数据...", "取消", 0, 100, self)
            ...
            QtWidgets.QMessageBox.information(
                ...
                "转换完成",
                f"TDX数据转换完成！\n\n"
                f"成功转换文件数量：{converted_count} 个\n\n"
                f"转换结果保存在：{target_dir}"
            )
            ...
            QtWidgets.QMessageBox.critical(
                self,
                "转换失败",
                f"转换TDX数据时出现错误：{str(e)}"
            )

    def _log_callback(self, msg: str) -> None:
        """日志回调函数，用于在转换过程中显示进度"""
        # 这里可以选择性地处理日志，比如只显示关键信息
        # 目前暂时不显示详细日志，避免界面过于复杂
        pass

    def batch_import_data(self) -> None:
        """批量导入TDX数据：转换 + 导入 + 聚合"""
        # 提供两种导入选项
        msg_box = QtWidgets.QMessageBox()
        msg_box.setWindowTitle("选择导入模式")
        msg_box.setText("请选择数据导入模式：")
        msg_box.setStandardButtons(QtWidgets.QMessageBox.Cancel)

        # 添加自定义按钮
        incremental_button = msg_box.addButton("增量导入", QtWidgets.QMessageBox.ActionRole)
        full_import_button = msg_box.addButton("全量导入", QtWidgets.QMessageBox.ActionRole)

        msg_box.setDefaultButton(incremental_button)
        msg_box.setInformativeText(
            "增量导入：只导入新增数据，保留现有数据\n"
            "全量导入：删除现有数据，重新导入所有数据\n\n"
            "导入流程：1.转换TDX数据 2.导入数据库 3.聚合高周期数据"
        )
        ...
        # 确定导入模式
        force_update = (msg_box.clickedButton() == full_import_button)
        mode_text = "全量导入" if force_update else "增量导入"

        try:
            # 显示总进度对话框
            progress = QtWidgets.QProgressDialog(f"正在执行{mode_text}...", "取消", 0, 100, self)
            progress.setWindowTitle("批量导入进度")
            progress.setWindowModality(QtCore.Qt.WindowModal)
            progress.setValue(0)
            progress.show()
            ...
            # 步骤2：导入数据库
            progress.setLabelText("步骤2/3：导入数据库...")
            progress.setValue(50)

            # 使用新的统一流水线进行导入和聚合
            result = self.engine.run_data_update_pipeline(
                source_dir=source_dir,
                target_dir=target_dir,
                auto_aggregate=True,
            ...
            # 合并统计信息
            total_stats["imported_contracts"] = result.get("imported_contracts", 0)
            total_stats["aggregated_hourly"] = result.get("aggregated_hourly", 0)
            total_stats["aggregated_daily"] = result.get("aggregated_daily", 0)

            progress.setValue(90)

            # 步骤3：完成
            progress.setLabelText("步骤3/3：完成")
            ...
            QtWidgets.QMessageBox.information(
                ...
                f"导入合约: {total_stats['imported_contracts']} 个\n"
                f"聚合小时线: {total_stats['aggregated_hourly']} 条\n"
                f"聚合日线: {total_stats['aggregated_daily']} 条\n\n"
                f"源目录: {source_dir}\n"
                f"目标目录: {target_dir}"
            )
